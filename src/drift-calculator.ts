...
export type StructureType =
  | "masonry_4stories_or_less"    // RC=I/II: 0.025hsx  (masonry ≤4 stories)
  | "other_4stories_or_less"      // RC=I/II: 0.020hsx  (most structures ≤4 stories)
  | "masonry_general"             // RC=I/II: 0.020hsx  (masonry >4 stories not elsewhere classified)
  | "all_other";                  // RC=I/II: 0.020hsx  (all other structures)

/** Seismic force-resisting systems with their Cd values per ASCE 7-22 Table 12.2-1 */
export interface SeismicSystem {
  name: string;
  cdMin: number;  // Lower bound of Cd (use cdMin == cdMax for fixed values)
  cdMax: number;  // Upper bound of Cd (same as cdMin when no range)
  description: string;
}

/** Input parameters for inter-story drift ratio calculation */
export interface DriftInput {
  deltaE: number;         // Elastic story displacement (inches), from ELFP or modal analysis
  hsx: number;            // Story height (inches) below the story under consideration
  cd: number;             // Deflection amplification factor (dimensionless)
  ie: number;             // Seismic importance factor (dimensionless)
...
export interface DriftResult {
  deltaX: number;         // Amplified story drift: δx = Cd × δxe / Ie  (inches)
  driftRatio: number;     // Inter-story drift ratio: Δ/hsx (dimensionless)
  allowableDrift: number; // Δa per Table 12.12-1 (inches)
  allowableRatio: number; // Δa/hsx (dimensionless)
  passes: boolean;        // true if Δ/hsx ≤ Δa/hsx
  utilizationRatio: number; // driftRatio / allowableRatio (0–∞; >1.0 = fail)
  summary: string;
}

/** Input parameters for SDC determination */
export interface SdcInput {
  ss: number;             // Mapped MCE spectral acceleration, short period (g), USGS/ASCE maps
  s1: number;             // Mapped MCE spectral acceleration, 1-sec period (g)
  siteClass: SiteClass;
  riskCategory: RiskCategory;
}
...
export interface SdcResult {
  fa: number;             // Short-period site coefficient (Table 11.4-1)
  fv: number;             // Long-period site coefficient (Table 11.4-2)
  sms: number;            // MCE short-period adjusted: SMS = Fa × Ss
  sm1: number;            // MCE 1-sec adjusted: SM1 = Fv × S1
  sds: number;            // Design spectral acceleration, short: SDS = (2/3) × SMS
  sd1: number;            // Design spectral acceleration, 1-sec:  SD1 = (2/3) × SM1
  sdcFromSds: SDC;        // SDC based on SDS per Table 11.6-1
  sdcFromSd1: SDC;        // SDC based on SD1 per Table 11.6-2
  sdc: SDC;               // Governing SDC (the more severe of the two)
  summary: string;
}

/** Input for P-Delta stability check */
export interface PDeltaInput {
  px: number;             // Total vertical design load above story x (kips)
  delta: number;          // Amplified story drift (inches): Δx from driftResult.deltaX
  ie: number;             // Seismic importance factor
  vx: number;             // Seismic story shear at story x (kips)
  hsx: number;            // Story height (inches)
  cd: number;             // Deflection amplification factor
  beta?: number;          // Ratio of shear demand/capacity at level x (§12.8.7); default 1.0
}

/** P-Delta stability coefficient result */
export interface PDeltaResult {
  theta: number;          // Stability coefficient θ
  thetaMax: number;       // Maximum permitted θ per §12.8.7
  requiresPDeltaAnalysis: boolean;   // true if θ > 0.10
  structurallyUnstable: boolean;     // true if θ > θmax
  summary: string;
}
 ...
 */
export const SEISMIC_SYSTEMS: Record<string, SeismicSystem> = {
  SMRF_STEEL: {
  ...
  SIMPSON_STRONG_WALL: {
    ...
    description:
      "Proprietary wood shear wall per ICC-ES ESR-1799/ESR-2652. " +
 ...
 */
export function getAllowableDrift(
  hsx: number,
  riskCategory: RiskCategory,
  structureType: StructureType
): number {
 ...
 */
export function calculateInterStoryDrift(input: DriftInput): DriftResult {
  const { deltaE, hsx, cd, ie, riskCategory, structureType } = input;

  if (hsx <= 0) throw new Error("Story height hsx must be > 0");
  if (cd <= 0)  throw new Error("Deflection amplification factor Cd must be > 0");
  if (ie <= 0)  throw new Error("Importance factor Ie must be > 0");
  if (deltaE < 0) throw new Error("Elastic displacement deltaE must be ≥ 0");
  ...
  const summary =
    `Story drift Δ = ${deltaX.toFixed(3)}" (${(driftRatio * 100).toFixed(3)}% of hsx). ` +
 ...
 */
export function getFa(ss: number, siteClass: SiteClass): number {
  if (siteClass === "F") {
  ...
  const classIndex: Record<SiteClass, number> = { A: 0, B: 1, C: 2, D: 3, E: 4, F: -1 };
 ...
 */
export function getFv(s1: number, siteClass: SiteClass): number {
  if (siteClass === "F") {
    throw new Error("Site Class F requires a site-specific hazard analysis per ASCE 7-22 §21.1");
  }
...
function interpolateTableValue(
  ...
  if (x <= table[0][0]) return table[0][col + 1];
  if (x >= table[n - 1][0]) return table[n - 1][col + 1];
...
function sdcFromSds(sds: number, rc: RiskCategory): SDC {
  if (rc === "I" || rc === "II") {
...
function sdcFromSd1(sd1: number, rc: RiskCategory): SDC {
  if (rc === "I" || rc === "II") {
...
function maxSdc(a: SDC, b: SDC): SDC {
 ...
 */
export function determineSDC(input: SdcInput): SdcResult {
  const { ss, s1, siteClass, riskCategory } = input;

  // Special rule for high S1 regions (§11.6)
  if (s1 >= 0.75) {
    const sdc: SDC = riskCategory === "IV" ? "F" : "E";
    ...
    return {
      ...
      sdc,
      summary:
        `S1 = ${s1} ≥ 0.75 g → SDC ${sdc} per ASCE 7-22 §11.6 special rule. ` +
 ...
 */
export function checkPDelta(input: PDeltaInput): PDeltaResult {
  const { px, delta, ie, vx, hsx, cd, beta = 1.0 } = input;

  if (vx <= 0)  throw new Error("Story shear Vx must be > 0");
  if (hsx <= 0) throw new Error("Story height hsx must be > 0");
 ...
 */
export function generateStoryReport(
  label: string,
  drift: DriftInput,
  pDelta?: PDeltaInput
): StoryReport {
